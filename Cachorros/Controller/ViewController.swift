    ...
    func populaArrayCachorrosAtualizaTableView(completion: @escaping (Result<[Cachorro], CachorroApiError>) -> Void) {
        ...
        if !favoritos {
            guard let mApi = self.api else { return }
                ...
                mApi.getCachorros(urlString: mApi.setCachorrosURL(), method: .GET) {
                    [weak self] result in
                    guard self != nil else { return }
            ...
            for item in cachorros {
              ...
              //  let dog = Cachorro(_id: item, name: item, wikilink: item, image: item, note: item)
                let dog = Cachorro(_id: item._id, name: item.name, wikilink: item.wikilink, image: item.image, note: item.note)
...
extension ViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: reuseIdentifier, for: indexPath) as? CelulaCustomizadaCachorroTableViewCell
